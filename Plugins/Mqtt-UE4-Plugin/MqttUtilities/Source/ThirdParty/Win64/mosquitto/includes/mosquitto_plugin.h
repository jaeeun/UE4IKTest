 ...
 *
 * This header contains function declarations for use when writing a Mosquitto
 * plugin.
 */
 ...
 * On Mac OS X:
 *
 * gcc -I<path to mosquitto_plugin.h> -fPIC -shared plugin.c -undefined
 * dynamic_lookup -o plugin.so
 *
 ...
 *
 * The supported_versions array tells you which plugin versions the broker
 * supports.
 *
 ...
 *	                 functions. Use to hold connection information for
 *example. opts -           Pointer to an array of struct mosquitto_opt, which
 *	                 provides the plugin options defined in the
 *configuration file. opt_count -      The number of elements in the opts array.
 *
 * Return value:
 *	Return 0 on success
 *	Return >0 on failure.
 ...
 *	                 provides the plugin options defined in the
 *configuration file. opt_count -      The number of elements in the opts array.
 *	reload -         If set to false, this is the first time the function
 *has been called. If true, the broker has received a signal asking to reload
 *its configuration.
 *
 * Return value:
 *	Return 0 on success
 ...
 *	Return >0 on failure.
 */
mosq_plugin_EXPORT int
mosquitto_auth_security_cleanup(void *user_data, struct mosquitto_opt *opts,
                                int opt_count, bool reload);
 ...
 *                     on the server.
 *  MOSQ_ACL_READ      when a message is about to be sent to a client (i.e.
 *whether it can read that topic or not). MOSQ_ACL_WRITE     when a message has
 *been received from a client (i.e. whether it can write to that topic or not).
 *
 * Return:
 ...
 */
mosq_plugin_EXPORT int
mosquitto_auth_acl_check(void *user_data, int access, struct mosquitto *client,
 ...
 *	Return 0 on success.
 *	Return >0 on failure.
 *	Return MOSQ_ERR_PLUGIN_DEFER if your plugin does not wish to handle this
 *check.
 */
 ...
 *	reauth - this is set to false if this is the first authentication
 *attempt on a connection, set to true if the client is attempting to
 *	         reauthenticate.
 ...
 *	Return MOSQ_ERR_AUTH_CONTINUE if the authentication is a multi step
 *process and can continue. Return MOSQ_ERR_AUTH if authentication was valid but
 *did not succeed. Return any other relevant positive integer MOSQ_ERR_* to
 *produce an error.
 */
mosq_plugin_EXPORT int
mosquitto_auth_start(void *user_data, struct mosquitto *client,
                     const char *method, bool reauth, const void *data_in,
                     uint16_t data_in_len, void **data_out,
                     uint16_t *data_out_len);

mosq_plugin_EXPORT int
mosquitto_auth_continue(void *user_data, struct mosquitto *client,
                        const char *method, const void *data_in,
                        uint16_t data_in_len, void **data_out,
                        uint16_t *data_out_len);
