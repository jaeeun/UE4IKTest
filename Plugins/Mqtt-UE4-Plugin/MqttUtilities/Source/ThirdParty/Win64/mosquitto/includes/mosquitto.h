 ...
 * File: mosquitto.h
 *
 * This header contains functions and definitions for use with libmosquitto, the
 * Mosquitto client library.
 *
 * The definitions are also used in Mosquitto broker plugins, and some functions
...
#define LIBMOSQUITTO_VERSION_NUMBER                                            \
  (LIBMOSQUITTO_MAJOR * 1000000 + LIBMOSQUITTO_MINOR * 1000 +                  \
 ...
 *
 *	If your application uses threads you must use <mosquitto_threaded_set>
 *to tell the library this is the case, otherwise it makes some optimisations
 ...
 *
 * Returns:
 *	LIBMOSQUITTO_VERSION_NUMBER - which is a unique number based on the
 *major, minor and revision values. See Also: <mosquitto_lib_cleanup>,
 *<mosquitto_lib_init>
 */
 ...
 * id will be generated. If id is NULL, clean_session must be true.
 ...
 * 	NULL on failure. Interrogate errno to determine the cause for the
 * failure:
...
libmosq_EXPORT int mosquitto_reinitialise(struct mosquitto *mosq,
                                          const char *id, bool clean_session,
                                          void *obj);
 ...
 *
 * It is valid to use this function for clients using all MQTT protocol
 * versions. If you need to set MQTT v5 Will properties, use
 * <mosquitto_will_set_v5> instead.
 *
 * Parameters:
 * 	mosq -       a valid mosquitto instance.
 * 	topic -      the topic on which to publish the will.
 * 	payloadlen - the size of the payload (bytes). Valid values are between 0
 * and 268,435,455. payload -    pointer to the data to send. If payloadlen > 0
 * this must be a valid memory location. qos -        integer value 0, 1 or 2
 * indicating the Quality of Service to be used for the will. retain -     set
 * to true to make the will a retained message.
 *
 * Returns:
 * 	MOSQ_ERR_SUCCESS -      on success.
 * 	MOSQ_ERR_INVAL -          if the input parameters were invalid.
 ...
 * 	mosq -       a valid mosquitto instance.
 * 	topic -      the topic on which to publish the will.
 * 	payloadlen - the size of the payload (bytes). Valid values are between
 *0 and 268,435,455. payload -    pointer to the data to send. If payloadlen > 0
 *this must be a valid memory location. qos -        integer value 0, 1 or 2
 *indicating the Quality of Service to be used for the will. retain -     set to
 *true to make the will a retained message. properties - list of MQTT 5
 *properties. Can be NULL. On success only, the property list becomes the
 *property of libmosquitto once this function is called and will be freed by the
 *library. The property list must be freed by the application on error.
 *
 * Returns:
 * 	MOSQ_ERR_SUCCESS -      on success.
 * 	MOSQ_ERR_INVAL -          if the input parameters were invalid.
 ...
 * 	MOSQ_ERR_PAYLOAD_SIZE -   if payloadlen is too large.
 * 	MOSQ_ERR_MALFORMED_UTF8 - if the topic is not valid UTF-8.
 * 	MOSQ_ERR_NOT_SUPPORTED -  if properties is not NULL and the client is
 *not using MQTT v5 MOSQ_ERR_PROTOCOL -       if a property is invalid for use
 *with wills. MOSQ_ERR_DUPLICATE_PROPERTY - if a property is duplicated where it
 *is forbidden.
 */
 ...
 *             authentication.
 * 	password - the password to send as a string. Set to NULL when username
 * is valid in order to send just a username.
 *
 * Returns:
 * 	MOSQ_ERR_SUCCESS - on success.
 * 	MOSQ_ERR_INVAL -   if the input parameters were invalid.
 ...
 *
 * It is valid to use this function for clients using all MQTT protocol
 * versions. If you need to set MQTT v5 CONNECT properties, use
 ...
 * PING message to the client if no other messages have been exchanged in that
 * time.
 *
 * Returns:
 * 	MOSQ_ERR_SUCCESS - on success.
 * 	MOSQ_ERR_INVAL -   if the input parameters were invalid, which could be
 * any of:
 * 	                   * mosq == NULL
 * 	                   * host == NULL
 * 	                   * port < 0
 * 	                   * keepalive < 5
 * 	MOSQ_ERR_ERRNO -   if a system call returned an error. The variable
 * errno contains the error code, even on Windows. Use strerror_r() where
 * available or FormatMessage() on Windows.
 *
 * See Also:
 * 	<mosquitto_connect_bind>, <mosquitto_connect_async>,
 * <mosquitto_reconnect>, <mosquitto_disconnect>, <mosquitto_tls_set>
 */
 ...
 * 	host -         the hostname or ip address of the broker to connect to.
 * 	port -         the network port to connect to. Usually 1883.
 * 	keepalive -    the number of seconds after which the broker should send
 * a PING message to the client if no other messages have been exchanged in that
 * time. bind_address - the hostname or ip address of the local network
 ...
 *
 * See Also:
 * 	<mosquitto_connect>, <mosquitto_connect_async>,
 ...
 * 	keepalive -    the number of seconds after which the broker should send
 *a PING message to the client if no other messages have been exchanged in that
 *time. bind_address - the hostname or ip address of the local network interface
 *to bind to. properties - the MQTT 5 properties for the connect (not for the
 *Will).
 *
 * Returns:
 * 	MOSQ_ERR_SUCCESS - on success.
 ...
 * 	MOSQ_ERR_ERRNO -   if a system call returned an error. The variable
 *errno contains the error code, even on Windows. Use strerror_r() where
 *available or FormatMessage() on Windows. MOSQ_ERR_DUPLICATE_PROPERTY - if a
 *property is duplicated where it is forbidden. MOSQ_ERR_PROTOCOL - if any
 *property is invalid for use with CONNECT.
 *
 * See Also:
 * 	<mosquitto_connect>, <mosquitto_connect_async>,
 *<mosquitto_connect_bind_async>
 */
libmosq_EXPORT int
mosquitto_connect_bind_v5(struct mosquitto *mosq, const char *host, int port,
 ...
 *
 * See Also:
 * 	<mosquitto_connect_bind_async>, <mosquitto_connect>,
 * <mosquitto_reconnect>, <mosquitto_disconnect>, <mosquitto_tls_set>
 */
 ...
 *
 * It is valid to use this function for clients using all MQTT protocol
 *versions. If you need to set MQTT v5 DISCONNECT properties, use
 ...
 * 	MOSQ_ERR_INVAL -   if the input parameters were invalid.
 * 	MOSQ_ERR_NO_CONN -  if the client isn't connected to a broker.
 *	MOSQ_ERR_DUPLICATE_PROPERTY - if a property is duplicated where it is
 *forbidden. MOSQ_ERR_PROTOCOL - if any property is invalid for use with
 *DISCONNECT.
 */
libmosq_EXPORT int
mosquitto_disconnect_v5(struct mosquitto *mosq, int reason_code,
                        const mosquitto_property *properties);
 ...
 *
 * It is valid to use this function for clients using all MQTT protocol
 *versions. If you need to set MQTT v5 PUBLISH properties, use
 *<mosquitto_publish_v5> instead.
 *
 * Parameters:
 * 	mosq -       a valid mosquitto instance.
 ...
 *0 and 268,435,455. payload -    pointer to the data to send. If payloadlen > 0
 *this must be a valid memory location. qos -        integer value 0, 1 or 2
 *indicating the Quality of Service to be used for the message. retain -     set
 *to true to make the message retained.
 *
 * Returns:
 * 	MOSQ_ERR_SUCCESS -        on success.
 * 	MOSQ_ERR_INVAL -          if the input parameters were invalid.
 * 	MOSQ_ERR_NOMEM -          if an out of memory condition occurred.
 * 	MOSQ_ERR_NO_CONN -        if the client isn't connected to a broker.
 *	MOSQ_ERR_PROTOCOL -       if there is a protocol error communicating
 *with the broker. MOSQ_ERR_PAYLOAD_SIZE -   if payloadlen is too large.
 * 	MOSQ_ERR_MALFORMED_UTF8 - if the topic is not valid UTF-8
 *	MOSQ_ERR_QOS_NOT_SUPPORTED - if the QoS is greater than that supported
 *by the broker. MOSQ_ERR_OVERSIZE_PACKET - if the resulting packet would be
 ...
 *this must be a valid memory location. qos -        integer value 0, 1 or 2
 *indicating the Quality of Service to be used for the message. retain -     set
 *to true to make the message retained. properties - a valid mosquitto_property
 *list, or NULL.
 *
 * Returns:
 * 	MOSQ_ERR_SUCCESS -        on success.
 ...
 *	MOSQ_ERR_DUPLICATE_PROPERTY - if a property is duplicated where it is
 *forbidden. MOSQ_ERR_PROTOCOL - if any property is invalid for use with
 *PUBLISH. MOSQ_ERR_QOS_NOT_SUPPORTED - if the QoS is greater than that
 *supported by the broker. MOSQ_ERR_OVERSIZE_PACKET - if the resulting packet
 *would be larger than supported by the broker.
 */
 ...
 *
 * It is valid to use this function for clients using all MQTT protocol
 *versions. If you need to set MQTT v5 SUBSCRIBE properties, use
 *<mosquitto_subscribe_v5> instead.
 *
 * Parameters:
 *	mosq - a valid mosquitto instance.
 ...
 *	       with the subscribe callback to determine when the message has
 *been sent. sub -  the subscription pattern. qos -  the requested Quality of
 *Service for this subscription.
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS -        on success.
 * 	MOSQ_ERR_INVAL -          if the input parameters were invalid.
 ...
 *	       with the subscribe callback to determine when the message has
 *been sent. sub -  the subscription pattern. qos -  the requested Quality of
 *Service for this subscription. options - options to apply to this
 *subscription, OR'd together. Set to 0 to use the default options, otherwise
 *choose from list of <mqtt5_sub_options> properties - a valid
 *mosquitto_property list, or NULL.
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS -        on success.
 * 	MOSQ_ERR_INVAL -          if the input parameters were invalid.
 ...
 *	MOSQ_ERR_DUPLICATE_PROPERTY - if a property is duplicated where it is
 *forbidden. MOSQ_ERR_PROTOCOL - if any property is invalid for use with
 *SUBSCRIBE. MOSQ_ERR_OVERSIZE_PACKET - if the resulting packet would be larger
 ...
 *	       the message id of this particular message. This can be then used
 *	       with the subscribe callback to determine when the message has
 *been sent. sub_count - the count of subscriptions to be made sub -  array of
 *sub_count pointers, each pointing to a subscription string. The "char *const
 **const" datatype ensures that neither the array of pointers nor the strings
 *that they point to are mutable. If you aren't familiar with this, just think
 *of it as a safer "char **", equivalent to "const char *" for a simple string
 *pointer. qos -  the requested Quality of Service for each subscription.
 *	options - options to apply to this subscription, OR'd together. This
 *	          argument is not used for MQTT v3 susbcriptions. Set to 0 to
 *use the default options, otherwise choose from list of <mqtt5_sub_options>
 * 	properties - a valid mosquitto_property list, or NULL. Only used with
 *MQTT v5 clients.
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS -        on success.
 ...
 */
libmosq_EXPORT int
mosquitto_subscribe_multiple(struct mosquitto *mosq, int *mid, int sub_count,
                             char *const *const sub, int qos, int options,
                             const mosquitto_property *properties);
 ...
 *	mid -  a pointer to an int. If not NULL, the function will set this to
 *	       the message id of this particular message. This can be then used
 *	       with the unsubscribe callback to determine when the message has
 *been sent. sub -  the unsubscription pattern.
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS -        on success.
 * 	MOSQ_ERR_INVAL -          if the input parameters were invalid.
 ...
 *
 * It is valid to use this function for clients using all MQTT protocol
 *versions. If you need to set MQTT v5 UNSUBSCRIBE properties, use
 ...
 *	       the message id of this particular message. This can be then used
 *	       with the unsubscribe callback to determine when the message has
 *been sent. sub -  the unsubscription pattern. properties - a valid
 *mosquitto_property list, or NULL. Only used with MQTT v5 clients.
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS -        on success.
 * 	MOSQ_ERR_INVAL -          if the input parameters were invalid.
 ...
 *	MOSQ_ERR_DUPLICATE_PROPERTY - if a property is duplicated where it is
 *forbidden. MOSQ_ERR_PROTOCOL - if any property is invalid for use with
 *UNSUBSCRIBE. MOSQ_ERR_OVERSIZE_PACKET - if the resulting packet would be
 *larger than supported by the broker.
 */
libmosq_EXPORT int
mosquitto_unsubscribe_v5(struct mosquitto *mosq, int *mid, const char *sub,
                         const mosquitto_property *properties);
 ...
 *	       the message id of this particular message. This can be then used
 *	       with the subscribe callback to determine when the message has
 *been sent. sub_count - the count of unsubscriptions to be made sub -  array of
 *sub_count pointers, each pointing to an unsubscription string. The "char
 **const *const" datatype ensures that neither the array of pointers nor the
 *strings that they point to are mutable. If you aren't familiar with this, just
 *think of it as a safer "char **", equivalent to "const char *" for a simple
 *string pointer. properties - a valid mosquitto_property list, or NULL. Only
 *used with MQTT v5 clients.
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS -        on success.
 * 	MOSQ_ERR_INVAL -          if the input parameters were invalid.
 ...
 */
libmosq_EXPORT int
mosquitto_unsubscribe_multiple(struct mosquitto *mosq, int *mid, int sub_count,
 ...
 */
libmosq_EXPORT void
mosquitto_message_free_contents(struct mosquitto_message *message);
 ...
 * Parameters:
 *  mosq - a valid mosquitto instance.
 *	timeout -     Maximum number of milliseconds to wait for network
 *activity in the select() call before timing out. Set to 0 for instant return.
 *Set negative to use the default of 1000ms. max_packets - this parameter is
 *currently unused and should be set to 1 for future compatibility.
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS -   on success.
 * 	MOSQ_ERR_INVAL -     if the input parameters were invalid.
 ...
 *
 * See Also:
 *	<mosquitto_connect_async>, <mosquitto_loop>, <mosquitto_loop_forever>,
 *<mosquitto_loop_stop>
 */
 ...
 * 	MOSQ_ERR_ERRNO -     if a system call returned an error. The variable
 *errno contains the error code, even on Windows. Use strerror_r() where
 *available or FormatMessage() on Windows. See Also: <mosquitto_loop_forever>,
 ...
 * Parameters:
 *	mosq -        a valid mosquitto instance.
 *	max_packets - this parameter is currently unused and should be set to 1
 ...
 *	          Setting to NULL means that libmosquitto will use its own
 *SSL_CTX if TLS is to be used. This option is only available for openssl 1.1.0
 *and higher.
 *
 ...
 *	          relevant to you. If you use this option then you must
 *configure the TLS options as normal, i.e. you should use <mosquitto_tls_set>
 *to configure the cafile/capath as a minimum. This option is only available for
 ...
 *
 *	MOSQ_OPT_PROTOCOL_VERSION - Value must be set to either
 *MQTT_PROTOCOL_V31, MQTT_PROTOCOL_V311, or MQTT_PROTOCOL_V5. Must be set before
 *the client connects.  Defaults to MQTT_PROTOCOL_V311.
 *
 *	MOSQ_OPT_RECEIVE_MAXIMUM - Value can be set between 1 and 65535
 *inclusive, and represents the maximum number of incoming QoS 1 and QoS 2
 *	          messages that this client wants to process at once. Defaults
 *to
 ...
 *	          will override this option. Using this option is the
 *recommended method however.
 *
 ...
 *	          then the user specified SSL_CTX passed in using
 *MOSQ_OPT_SSL_CTX will have the default options applied to it. This means that
 ...
 *	          trust OS provided CA certificates for use with TLS
 *connections. Set to 0 (the default) to only use manually specified CA certs.
 */
 ...
 *
 *	MOSQ_OPT_BIND_ADDRESS - Set the hostname or ip address of the local
 *network interface to bind to when connecting.
 */
 ...
 *	          connections rather than libmosquitto creating its own.  This
 *must be called before connecting to have any effect. If you use this option,
 *the onus is on you to ensure that you are using secure settings. Setting to
 *NULL means that libmosquitto will use its own SSL_CTX if TLS is to be used.
 *	          This option is only available for openssl 1.1.0 and higher.
 */
 ...
 * Options:
 *	MOSQ_OPT_TLS_ENGINE
 *	          Configure the client for TLS Engine support. Pass a TLS Engine
 *ID to be used when creating TLS connections. Must be set before
 *<mosquitto_connect>. MOSQ_OPT_TLS_KEYFORM Configure the client to treat the
 *keyfile differently depending on its type.  Must be set before
 *<mosquitto_connect>. Set as either "pem" or "engine", to determine from where
 *the private key for a TLS connection will be obtained. Defaults to "pem", a
 *normal private key file. MOSQ_OPT_TLS_KPASS_SHA1 Where the TLS Engine requires
 *the use of a password to be accessed, this option allows a hex encoded SHA1
 *hash of the private key password to be passed to the engine directly. Must be
 *set before <mosquitto_connect>. MOSQ_OPT_TLS_ALPN If the broker being
 *connected to has multiple services available on a single TLS port, such as
 *both MQTT and WebSockets, use this option to configure the ALPN option for the
 *connection.
 */
libmosq_EXPORT int mosquitto_string_option(struct mosquitto *mosq,
                                           enum mosq_opt_t option,
...
libmosq_EXPORT int mosquitto_reconnect_delay_set(
    struct mosquitto *mosq, unsigned int reconnect_delay,
 ...
 */
libmosq_EXPORT int
mosquitto_max_inflight_messages_set(struct mosquitto *mosq,
...
libmosq_EXPORT int mosquitto_tls_set(
    struct mosquitto *mosq, const char *cafile, const char *capath,
 ...
 * Parameters:
 *  mosq -        a valid mosquitto instance.
 *	cert_reqs -   an integer defining the verification requirements the
 *client will impose on the server. This can be one of:
 *	              * SSL_VERIFY_NONE (0): the server will not be verified in
 *any way.
 *	              * SSL_VERIFY_PEER (1): the server certificate will be
 *verified and the connection aborted if the verification fails. The default and
 *recommended value is SSL_VERIFY_PEER. Using SSL_VERIFY_NONE provides no
 *security. tls_version - the version of the SSL/TLS protocol to use as a
 *string. If NULL, the default value is used. The default value and the
 *	              available values depend on the version of openssl that the
 *	              library was compiled against. For openssl >= 1.0.1, the
 *	              available options are tlsv1.2, tlsv1.1 and tlsv1, with
 *tlv1.2 as the default. For openssl < 1.0.1, only tlsv1 is available. ciphers -
 *a string describing the ciphers available for use. See the "openssl ciphers"
 *tool for more information. If NULL, the default ciphers will be used.
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS - on success.
 * 	MOSQ_ERR_INVAL -   if the input parameters were invalid.
 ...
 *	ciphers -  a string describing the PSK ciphers available for use. See
 *the "openssl ciphers" tool for more information. If NULL, the default ciphers
 *will be used.
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS - on success.
...
libmosq_EXPORT int mosquitto_tls_psk_set(struct mosquitto *mosq,
                                         const char *psk, const char *identity,
                                         const char *ciphers);
 ...
 *        For MQTT v5.0, look at section 3.2.2.2 Connect Reason code:
 * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html For MQTT
 * v3.1.1, look at section 3.2.2.3 Connect Return code:
 ...
 * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html For MQTT
 * v3.1.1, look at section 3.2.2.3 Connect Return code:
 * http://docs.oasis-open.org/mqtt/mqtt/v3.1.1/mqtt-v3.1.1.html flags - the
 * connect flags.
 */
 ...
 * v3.1.1, look at section 3.2.2.3 Connect Return code:
 * http://docs.oasis-open.org/mqtt/mqtt/v3.1.1/mqtt-v3.1.1.html flags - the
 * connect flags. props - list of MQTT 5 properties, or NULL
 *
 */
 ...
 *               void callback(struct mosquitto *mosq, void *obj, const struct
 ...
 *                 void callback(struct mosquitto *mosq, void *obj, int mid, int
 ...
 *           void callback(struct mosquitto *mosq, void *obj, int level, const
 *char *str)
 *
 * Callback Parameters:
 *  mosq -  the mosquitto instance making the callback.
 ...
 *   username - if not NULL, use this username when authenticating with the
 * proxy. password - if not NULL and username is not NULL, use this password
 * when authenticating with the proxy.
 */
 ...
 *	count -    an int pointer to store the number of items in the topics
 *array.
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS -        on success
 ...
 *   MOSQ_ERR_INVAL -          if the topic contains a + or a #, or if it is too
 * long. MOSQ_ERR_MALFORMED_UTF8 - if topic is not valid UTF-8
 *
 * See Also:
...
libmosq_EXPORT int mosquitto_subscribe_simple(
    struct mosquitto_message **messages, int msg_count, bool want_retained,
    const char *topic, int qos, const char *host, int port,
    const char *client_id, int keepalive, bool clean_session,
 ...
 *              int callback(struct mosquitto *mosq, void *obj, const struct
 * mosquitto_message *message) Note that this is the same as the normal
 * on_message callback, except that it returns an int. userdata - user provided
 * pointer that will be passed to the callback. topic - the subscription topic
 * to use (wildcards are allowed). qos - the qos to use for the subscription.
 *   host - the broker to connect to.
 *   port - the network port the broker is listening on.
 *   client_id - the client id to use, or NULL if a random client id should be
 *               generated.
...
libmosq_EXPORT int mosquitto_subscribe_callback(
    int (*callback)(struct mosquitto *, void *,
                    const struct mosquitto_message *),
    void *userdata, const char *topic, int qos, const char *host, int port,
    const char *client_id, int keepalive, bool clean_session,
    const char *username, const char *password,
    const struct libmosquitto_will *will, const struct libmosquitto_tls *tls);
 ...
 *	identifier - property identifier (e.g.
 *MQTT_PROP_PAYLOAD_FORMAT_INDICATOR) value - integer value for the new property
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS - on success
 ...
 * > mosquitto_property_add_byte(&proplist, MQTT_PROP_PAYLOAD_FORMAT_IDENTIFIER,
 *1);
 */
 ...
 *	proplist - pointer to mosquitto_property pointer, the list of properties
 *	identifier - property identifier (e.g.
 *MQTT_PROP_MESSAGE_EXPIRY_INTERVAL) value - integer value for the new property
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS - on success
 *	MOSQ_ERR_INVAL - if identifier is invalid, or if proplist is NULL
 ...
 *	proplist - pointer to mosquitto_property pointer, the list of properties
 *	identifier - property identifier (e.g.
 *MQTT_PROP_SUBSCRIPTION_IDENTIFIER) value - integer value for the new property
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS - on success
 *	MOSQ_ERR_INVAL - if identifier is invalid, or if proplist is NULL
 ...
 * Example:
 * > mosquitto_property *proplist = NULL;
 * > mosquitto_property_add_varint(&proplist, MQTT_PROP_SUBSCRIPTION_IDENTIFIER,
 *1);
 */
 ...
 *	proplist - pointer to mosquitto_property pointer, the list of properties
 *	identifier - property identifier (e.g.
 *MQTT_PROP_PAYLOAD_FORMAT_INDICATOR) value - pointer to the property data len -
 ...
 * Example:
 * > mosquitto_property *proplist = NULL;
 * > mosquitto_property_add_binary(&proplist, MQTT_PROP_AUTHENTICATION_DATA,
 ...
 *	value - string value for the new property, must be UTF-8 and zero
 *terminated
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS - on success
 *	MOSQ_ERR_INVAL - if identifier is invalid, if value is NULL, or if
 *proplist is NULL MOSQ_ERR_NOMEM - on out of memory MOSQ_ERR_MALFORMED_UTF8 -
 *value is not valid UTF-8.
 *
 * Example:
 * > mosquitto_property *proplist = NULL;
 * > mosquitto_property_add_string(&proplist, MQTT_PROP_CONTENT_TYPE,
 *"application/json");
 */
 ...
 *	name - string name for the new property, must be UTF-8 and zero
 *terminated value - string value for the new property, must be UTF-8 and zero
 *terminated
 *
 * Returns:
 *	MOSQ_ERR_SUCCESS - on success
 *	MOSQ_ERR_INVAL - if identifier is invalid, if name or value is NULL, or
 *if proplist is NULL MOSQ_ERR_NOMEM - on out of memory MOSQ_ERR_MALFORMED_UTF8
 *- if name or value are not valid UTF-8.
 *
 * Example:
 * > mosquitto_property *proplist = NULL;
 * > mosquitto_property_add_string_pair(&proplist, MQTT_PROP_USER_PROPERTY,
 *"client", "mosquitto_pub");
 */
libmosq_EXPORT int
mosquitto_property_add_string_pair(mosquitto_property **proplist,
 ...
 */
libmosq_EXPORT int
mosquitto_property_identifier(const mosquitto_property *property);
...
mosquitto_property_next(const mosquitto_property *proplist);
 ...
 * mosquitto_property_read_int16(proplist, MQTT_PROP_SERVER_KEEP_ALIVE,
 *&keepalive, false); (end)
 *
 * Parameters:
 *	proplist - mosquitto_property pointer, the list of properties or single
 *property identifier - property identifier (e.g.
 *MQTT_PROP_PAYLOAD_FORMAT_INDICATOR) value - pointer to store the value, or
 *NULL if the value is not required. skip_first - boolean that indicates whether
 *the first item in the list should be ignored or not. Should usually be set to
 *false.
 *
 * Returns:
 ...
 *	prop = mosquitto_property_read_byte(proplist, identifier, &value,
 *false); while(prop){ printf("value: %s\n", value); prop =
 *mosquitto_property_read_byte(prop, identifier, &value);
 *	}
 * (end)
 */
libmosq_EXPORT const mosquitto_property *
mosquitto_property_read_byte(const mosquitto_property *proplist, int identifier,
                             uint8_t *value, bool skip_first);
 ...
 */
libmosq_EXPORT const mosquitto_property *
mosquitto_property_read_int16(const mosquitto_property *proplist,
                              int identifier, uint16_t *value, bool skip_first);
 ...
 */
libmosq_EXPORT const mosquitto_property *
mosquitto_property_read_int32(const mosquitto_property *proplist,
                              int identifier, uint32_t *value, bool skip_first);
 ...
 */
libmosq_EXPORT const mosquitto_property *
mosquitto_property_read_varint(const mosquitto_property *proplist,
                               int identifier, uint32_t *value,
                               bool skip_first);
 ...
 * Returns:
 *	A valid property pointer if the property is found
 *	NULL, if the property is not found, or proplist is NULL, or if an out of
 ...
 */
libmosq_EXPORT const mosquitto_property *
mosquitto_property_read_binary(const mosquitto_property *proplist,
                               int identifier, void **value, uint16_t *len,
                               bool skip_first);
 ...
 *	property - property to read
 *	identifier - property identifier (e.g.
 *MQTT_PROP_PAYLOAD_FORMAT_INDICATOR) value - pointer to char*, for the property
 *data to be stored in, or NULL if the value is not required. skip_first -
 *boolean that indicates whether the first item in the list should be ignored or
 *not. Should usually be set to false.
 *
 * Returns:
 *	A valid property pointer if the property is found
 *	NULL, if the property is not found, or proplist is NULL, or if an out of
 ...
 */
libmosq_EXPORT const mosquitto_property *
mosquitto_property_read_string(const mosquitto_property *proplist,
                               int identifier, char **value, bool skip_first);
 ...
 *	property - property to read
 *	identifier - property identifier (e.g.
 *MQTT_PROP_PAYLOAD_FORMAT_INDICATOR) name - pointer to char* for the name
 *property data to be stored in, or NULL if the name is not required. value -
 *pointer to char*, for the property data to be stored in, or NULL if the value
 *is not required. skip_first - boolean that indicates whether the first item in
 *the list should be ignored or not. Should usually be set to false.
 *
 * Returns:
 *	A valid property pointer if the property is found
 *	NULL, if the property is not found, or proplist is NULL, or if an out of
 ...
 */
libmosq_EXPORT const mosquitto_property *
mosquitto_property_read_string_pair(const mosquitto_property *proplist,
                                    int identifier, char **name, char **value,
                                    bool skip_first);

/*
 * Function: mosquitto_property_free_all
 *
 * Free all properties from a list of properties. Frees the list and sets
 * *properties to NULL.
 *
 * Parameters:
 ...
 */
libmosq_EXPORT void
mosquitto_property_free_all(mosquitto_property **properties);
 ...
 *	MOSQ_ERR_DUPLICATE_PROPERTY - if a property is duplicated where it is
 *forbidden. MOSQ_ERR_PROTOCOL - if any property is invalid
 */
libmosq_EXPORT int
mosquitto_property_check_all(int command, const mosquitto_property *properties);
 ...
 *  NULL on failure
 */
libmosq_EXPORT const char *
mosquitto_property_identifier_to_string(int identifier);

/* Function: mosquitto_string_to_property_info
 *
 * Parse a property name string and convert to a property identifier and data
 *type. The property name is as defined in the MQTT specification, with - as a
 * separator, for example: payload-format-indicator.
 *
 * Parameters:
