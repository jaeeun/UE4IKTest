 ...
 *	MOSQ_ERR_ALREADY_EXISTS - if cb_func has already been registered for
 *this event MOSQ_ERR_NOT_SUPPORTED - if the event is not supported
 */
 ...
 */
mosq_EXPORT int
mosquitto_callback_unregister(mosquitto_plugin_id_t *identifier, int event,
                              MOSQ_FUNC_generic_callback cb_func,
                              const void *event_data);
 ...
 *
 * Retrieve the MQTT protocol version with which the client has connected. Can
 * be one of:
 *
 * Returns:
 *   3 - for MQTT v3 / v3.1
 ...
 */
mosq_EXPORT int
mosquitto_client_protocol_version(const struct mosquitto *client);
 ...
 */
mosq_EXPORT const char *
mosquitto_client_username(const struct mosquitto *client);
 ...
 *
 * Parameters:
 *  clientid -   optional string. If set to NULL, the message is delivered to
 * all clients. If non-NULL, the message is delivered only to the client with
 * the corresponding client id. If the client id specified is not connected, the
 * message will be dropped. topic -      message topic payloadlen - payload
 * length in bytes. Can be 0 for an empty payload. payload -    payload bytes.
 * If payloadlen > 0 this must not be NULL. Must be allocated on the heap. Will
 * be freed by mosquitto after use if the function returns success. qos -
 * message QoS to use. retain -     should retain be set on the message. This
 * does not apply if clientid is non-NULL. properties - MQTT v5 properties to
 * attach to the message. If the function returns success, then properties is
 * owned by the broker and will be freed at a later point.
 *
 * Returns:
 *   MOSQ_ERR_SUCCESS - on success
 ...
 * Parameters:
 *  clientid -   optional string. If set to NULL, the message is delivered to
 *all clients. If non-NULL, the message is delivered only to the client with the
 *corresponding client id. If the client id specified is not connected, the
 *message will be dropped. topic -      message topic payloadlen - payload
 *length in bytes. Can be 0 for an empty payload. payload -    payload bytes. If
 *payloadlen > 0 this must not be NULL. Memory remains the property of the
 *calling function. qos -        message QoS to use. retain -     should retain
 *be set on the message. This does not apply if clientid is non-NULL. properties
 *- MQTT v5 properties to attach to the message. If the function returns
 *success, then properties is owned by the broker and will be freed at a later
 *point.
 *
 * Returns:
 *   MOSQ_ERR_SUCCESS - on success
