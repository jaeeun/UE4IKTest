 ...
 * 	MOSQ_ERR_NOMEM -        if an out of memory condition occurred.
 * 	MOSQ_ERR_NO_CONN -      if the client isn't connected to a broker.
 *	MOSQ_ERR_PROTOCOL -     if there is a protocol error communicating with
 *the broker. MOSQ_ERR_PAYLOAD_SIZE - if payloadlen is too large.
 *
 * See Also:
 ...
 *	                            MQTT_PROTOCOL_V31 or MQTT_PROTOCOL_V311.
 *Must be set before the client connects. Defaults to MQTT_PROTOCOL_V31.
 */
