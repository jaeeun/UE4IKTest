...
public class MqttUtilities : ModuleRules
{
    public MqttUtilities(ReadOnlyTargetRules Target) : base(Target)
    {
        ...
        if (Target.Platform == UnrealTargetPlatform.Win64)
        {
            ...
            // FlatBuffer
            PublicIncludePaths.Add(Path.Combine(Path.Combine(ModuleDirectory, "../ThirdParty", Target.Platform.ToString()), "flatbuffers", "include"));
        }
    ...
    public void LoadThirdPartyLibrary(string libname, ReadOnlyTargetRules Target)
    {
        ...
        string[] file_paths = Directory.GetFiles(LibrariesPath, "*" + DynamicLibExtension);
        foreach (string file_path in file_paths)
        {
            string file_name = Path.GetFileName(file_path);
            if (!File.Exists(Path.Combine(BinariesPath, file_name)))
            {
                File.Copy(Path.Combine(LibrariesPath, file_name), Path.Combine(BinariesPath, file_name), true);
            }
        }
