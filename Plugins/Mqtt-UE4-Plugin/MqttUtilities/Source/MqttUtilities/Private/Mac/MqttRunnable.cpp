...
FMqttRunnable::FMqttRunnable(UMqttClient *mqttClient)
    : FRunnable(), TaskQueue(new std::queue<FMqttTaskPtr>()),
      TaskQueueLock(new FCriticalSection()), client(mqttClient) {}
  ...
  while (bKeepRunning) {
    ...
    while (TaskQueue != nullptr) {
      ...
      case MqttTaskType::Subscribe: {
        ...
        returnCode =
            connection.subscribe(NULL, taskSubscribe->sub, taskSubscribe->qos);
        break;
      }
