...
jbyteArray
ConversionUtils::ConvertToJByteArray(const TArray<uint8> &byteArray) {
...
FJavaClassObject *ConversionUtils::ConvertToJavaConnectionData(
    FMqttConnectionData connectionData) {
  FJavaClassObject *javaConnectionData = new FJavaClassObject(
      "org/eclipse/paho/client/mqttv3/MqttConnectOptions", "()V");
  FJavaClassMethod setUserNameMethod = javaConnectionData->GetClassMethod(
      "setUserName", "(Ljava/lang/String;)V");
  FJavaClassMethod setPasswordMethod =
      javaConnectionData->GetClassMethod("setPassword", "([C)V");

  javaConnectionData->CallMethod<void>(setUserNameMethod,
                                       GetJavaString(connectionData.Login));
...
FJavaClassObject *ConversionUtils::ConvertToJavaMessage(FMqttMessage message) {
  ...
  jbyteArray javaArray =
      (jbyteArray)Env->CallObjectMethod(javaMessageString, mid);

  FJavaClassObject *javaMessage = new FJavaClassObject(
      "org/eclipse/paho/client/mqttv3/MqttMessage", "([B)V", javaArray);

  FJavaClassMethod setQosMethod = javaMessage->GetClassMethod("setQos", "(I)V");
  FJavaClassMethod setRetainedMethod =
      javaMessage->GetClassMethod("setRetained", "(Z)V");

  return javaMessage;
}

FMqttMessage ConversionUtils::ConvertFromJavaMessage(jstring topic,
                                                     jobject javaMessage) {
  ...
  jmethodID GetPayloadMethodId =
      FJavaWrapper::FindMethod(Env, Class, "getPayload", "()[B", bIsOptional);
  jbyteArray javaPayload =
      (jbyteArray)Env->CallObjectMethod(javaMessage, GetPayloadMethodId);

  jclass helperClass = Env->FindClass("com/ninevastudios/mqttdemo/MqttHelper");
  jmethodID convertArrayMethod =
      FJavaWrapper::FindStaticMethod(Env, helperClass, "stringFromByteArray",
                                     "([B)Ljava/lang/String;", bIsOptional);
  jstring javaPayloadString = (jstring)Env->CallStaticObjectMethod(
      helperClass, convertArrayMethod, javaPayload);
  message.Message = FromJavaString(javaPayloadString);

  jmethodID GetQosMethodId =
      FJavaWrapper::FindMethod(Env, Class, "getQos", "()I", bIsOptional);
  message.Qos = Env->CallIntMethod(javaMessage, GetQosMethodId);

  jmethodID GetRetainedMethodId =
      FJavaWrapper::FindMethod(Env, Class, "isRetained", "()Z", bIsOptional);
